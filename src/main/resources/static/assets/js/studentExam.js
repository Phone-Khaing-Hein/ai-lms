const app = Vue.createApp({
  ...
  methods: {
    ...
    getStudentHasExam() {
      axios
        .get(
          `http://localhost:9090/api/studentExam/getStudentHasExam/${this.studentId}/${this.examId}`
        )
        .then((res) => {
    ...
    handleSubmit() {
      if (this.studentAnswerList.length > 0) {
        ...
        this.exam.questions.forEach((question) => {
          rightAnswerList = [
            ...rightAnswerList,
            [question.correctAnswer, question.mark],
          ];
        });
        ...
        axios.post(
          `http://localhost:9090/api/studentExam/addStudentHasExam`,
          null,
          {
            params: {
